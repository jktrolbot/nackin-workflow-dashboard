...
export default function ExecutionHistory({ executions, limit = 10 }: ExecutionHistoryProps) {
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: "1px solid #1e2535" }}>
      ...
      <div className="flex items-center justify-between px-5 py-4"
        style={{ background: "#0f1420", borderBottom: "1px solid #1e2535" }}>
        <div>
        ...
        <div className="flex gap-2">
          {(["success", "failed", "running"] as ExecutionStatus[]).map(s => {
            ...
            return count > 0 ? (
              <span key={s} className="text-xs px-2 py-1 rounded-md flex items-center gap-1"
                style={{ color: cfg.color, background: `${cfg.color}15` }}>
                <span style={{ fontFamily: "var(--font-mono)" }}>{count}</span> {s}
              </span>
            ) : null;
          })}
        </div>
      ...
      <div style={{ background: "#0a0e1a" }}>
        ...
        <div className="grid px-5 py-2.5"
          style={{ gridTemplateColumns: "1fr 120px 100px 80px 100px", borderBottom: "1px solid #1e2535" }}>
          {["Execution ID", "Started", "Duration", "Items", "Triggered By"].map(h => (
            <span key={h} className="text-xs font-medium" style={{ color: "#64748b" }}>{h}</span>
          ))}
        </div>

        {/* Rows */}
        <div className="divide-y" style={{ borderColor: "#1e2535" }}>
          {displayed.map((exec, i) => {
            ...
            return (
              <div
                key={exec.id}
                className={cn(
                  "grid px-5 py-3 transition-colors items-center",
                  "hover:bg-white/[0.02]"
                )}
              ...
              >
                ...
                <div className="flex items-center gap-2 min-w-0">
                  <StatusIcon
                    className={cn("w-4 h-4 flex-shrink-0", exec.status === "running" && "animate-spin")}
                ...
                <div>
                  ...
                  <div className="text-xs mt-0.5" style={{ color: "#64748b", fontFamily: "var(--font-mono)" }}>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs" style={{ color: "#e2e8f0", fontFamily: "var(--font-mono)" }}>
