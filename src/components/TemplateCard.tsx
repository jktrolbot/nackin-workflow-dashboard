...
import { Clock, Users, ChevronRight, Star, Zap } from "lucide-react";
...
export default function TemplateCard({ template, index }: TemplateCardProps) {
  ...
  return (
    <div
      className={`rounded-xl p-5 flex flex-col animate-fade-in-up stagger-${Math.min(index + 1, 6)} group`}
      style={{
        background: "#0f1420",
    ...
    >
      ...
      {/* Description */}
      <p className="text-xs mb-4 flex-1 leading-relaxed" style={{ color: "#64748b" }}>
      ...
      <div className="flex items-center gap-1 mb-4 flex-wrap">
        {template.nodes.map((node, i) => (
          <div key={node.id} className="flex items-center gap-1">
            ...
            >
              <span>{node.icon}</span>
              <span className="hidden sm:inline" style={{ color: "#94a3b8", fontSize: "10px" }}>
                {node.label}
              </span>
            </div>
      ...
      <Button
        ...
        onMouseEnter={e => {
          (e.currentTarget as HTMLElement).style.background = "linear-gradient(135deg, rgba(99,102,241,0.25), rgba(16,185,129,0.15))";
        }}
        onMouseLeave={e => {
          (e.currentTarget as HTMLElement).style.background = "linear-gradient(135deg, rgba(99,102,241,0.15), rgba(16,185,129,0.1))";
        }}
      >
